...
1. On the left panel, select **workspaces (1)**, and then select **My Workspace (2)**.

    ![](../media/GL2-T2-S2.png)

1. To upload a Power BI Desktop file, click on **Upload (1)** and in the dropdown menu, select **Browse (2)**.

   > **Note:** Please make sure you are **logged into the VM** for the following steps.

    ![](../media/GL2-T2-S3.png)

1.	In the **Open** window, navigate to the **C:\LabFiles\artifacts\reports (1)** folder, select the **AnalyticsDemoReport.pbix (2)** file and **Open (3)** It.

  	![](../media/openfile.png)
   
1. Notice that a Power BI report and dataset have been added to your workspace.

   ![](../media/workspace.png)

1. To open the report, select the **AnalyticsDemoReport**.
...
1. This report has three sections:
...
1. Lets navigate to the **Churn Analysis Tab,** where we analyze Customer Churn. This report, along with Campaign Analytics and Website Analytics reports in Power BI, are coming from the data Lakehouse.

    ![](../media/GL2-T2-S8.png)

1. In the Scatter Chart on the left, the black dots represent customers more likely to churn, and the blue dots represent customers less likely to churn. Notice that when customer tenure is low and their spend amount is low, the customers are more likely to churn.
With this insight, Wide World Importers decides to target customers with lesser tenure and lesser spend amounts through their new campaigns.
    ...
    Now, let's go the Campaign Analyticss. 

1. Select the **Campaign Analytics** tab from the top right pane to navigate to the Campaign Analytics report.

    ![Select Campaigns Analytics](https://github.com/CloudLabsAI-Azure/Ignite-lab/blob/main/media/image3238.png?raw=true)

1. In this Campaign Analytics report, the Bar chart shows that the most popular campaigns launched are **gogreen** and **sustainablefashion**. 

    ![Campaigns report - Bar chart](https://github.com/CloudLabsAI-Azure/Ignite-lab/blob/main/media/image3239.png?raw=true)
   
1. Select **Website Analytics** from the top right pane to navigate to the Website Analytics Report.
...
1. Click on **Not Happy** on the Donut chart to filter the report page.
...
1.  We know that the millenials from the biggest customer segment are "Not Happy" when...
...
1. Select **Top Segments** to show the details.
...
1. Click on the first bubble and see the details for Segment 1.
...
7. In the **Connect to** dropdown list, ensure that **Built-in (1)** is selected. In the **Use database** dropdown list, ensure that **AnalyticsServerlessPool (2)** is selected.
...
8. To query the first 100 campaigns, in the script file, **select lines 5-12 (1)** ,Select **Run (2)**, and review the **query result (3)** in the lower pane.
...
9. To create a view that queries the first 100 campaigns, **select lines 35-44 (1)** in the script file and select **Run (2)** to create a view of the Campaign file in Serverless pool.

    ![](../media/midp-lab3-1.png)
...
10. From the left pane, select **Data (1)**, then expand **AnalyticsServerlessPool (SQL) (3)** under SQL **Database (2)**. Next, expand **Views (4)**, followed by **dbo.Vw_CampaignData (5)**, and finally, expand **Columns (6)** to review the **available columns (7)**.
...
In this exercise, machine learning and business intelligence tasks were carried out on a Lakehouse environment. The process began with the deployment of machine learning models for customer churn prediction and sentiment analysis using Azure Databricks and Azure Machine Learning. These models were operationalized and integrated into business processes such as campaign analytics and sales forecasting. Finally, insights were derived from the Lakehouse data using Power BI, providing valuable business intelligence for decision-making and performance optimization.
