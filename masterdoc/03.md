...
This will take you to a webpage that confirms the **Data Simulation** has started:
...
8.	In the filter box for resources, search for **Synapse**.
...
>**Note:** You may see a Synapse workspace resource name with a different suffix in your Azure Portal.

10. In the Open Synapse Studio tile, select Open link.


![Open Synapse studio](https://github.com/CloudLabsAI-Azure/Ignite-lab/blob/main/media/image1115.png?raw=true)

>**Note:** Synapse Studio will open in a new web session (tab).

11.	On the left pane in Synapse Studio, select the **Data** hub icon.
...
*An ADX pool has already been created for this lab in the Azure Synapse workspace.*

*By using ADX’s powerful Kusto Query Language (KQL), you can be sure that the thresholds you set for each device in the store are being met.*
...
16.	On the left pane in Azure Data Explorer Studio, select the **Data** hub icon.

>**Note:** Select **Dismiss** if any pop-ups appear on your screen.

17.	On the **Data Management** page, select the **Ingest data** action.

![Paste the URI into the address bar](https://github.com/CloudLabsAI-Azure/Ignite-lab/blob/main/media/image1127.png?raw=true)

18.	On the **Destination** tab, in the **Cluster** dropdown list, select the Data Explorer pool starting with **analyticspool<inject key="DeploymentId"></inject>**.
...
![Add Cluster](https://github.com/CloudLabsAI-Azure/Ignite-lab/blob/main/media/imageAddCluster.png?raw=true)
  ...
  a. Navigate to the Azure Synapse web session (tab), and select **Manage** hub from the left pane.
...
![Copy URI](https://github.com/CloudLabsAI-Azure/Ignite-lab/blob/main/media/img120d.png?raw=true)
  ...
  e. Go back to the **Azure Data Explorer** tab in the **Connection URL** and enter the URL (copied above).

  f. Select **Add** and continue with Step 21.

![click Add](https://github.com/CloudLabsAI-Azure/Ignite-lab/blob/main/media/img120f.png?raw=true)

21.	On the **Source** tab in the **Source** dropdown list, select **Event Hub**.

22.	In the **Subscription** dropdown list, select your subscription.
...
27.	Select **More Parameters** in the **Compression** dropdown list and select **None**.
...
33.	Confirm that the continuous ingestion from the Event Hub has been established and then select **Close** (located at the bottom of the page).
...
35. In Synapse Studio, to the left, select the **Develop** hub icon (the third from the top).
...
38. In the **Connect to** dropdown list (if you don't see this option, click on the ellipsis [...] next to Publish on the top bar), and select the data explorer pool starting with **analyticspool-**.

>**Note:** If required, collapse the panes on the left using the << icon on the top right of each pane.

39. In the **Use database** dropdown list, select **AnalyticsDB**.

40.	Select the query (lines 4-8), displayed as **What is the average temp every minute?**.

*The query retrieves the average temperature per minute for a thermostat device (TH005) for the Miami store.*

41.	Select **Run**. 

42.	In the **Results** pane (located along the bottom), review the query result expressed as a chart. Please note that it may take 2-3 minutes to accumulate the data. If you don't see result, re-run query after awhile. 

>**Note:** If your query doesn't run successfully, chances are that the thermostat table was not created successfully in the previous steps. You may have to create that table with a different name e.g. Thermostat1, update the KQL query accordingly, and re-execute the KQL query.  

![Review the query result ](https://github.com/CloudLabsAI-Azure/Ignite-lab/blob/main/media/img_graph1.png?raw=true)

*Your graph may appear slightly different from the one shown above. It may take up to 60 seconds to load.*
...
#### Task 1.2: Explore some Synapse pipelines that ingest raw data from analytical data sources to the Bronze layer of the Data Lake. <a name="analytical-sources"></a>
...
2. In Synapse Studio, on the left, select the **Integrate** hub icon (fourth from the top).
